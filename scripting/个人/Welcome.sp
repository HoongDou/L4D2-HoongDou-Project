...
{   
	...
	RegConsoleCmd("sm_ip", ipCommand,"查看自己信息");
	
	SwitchTeamDEnabled = CreateConVar("l4d_switchteamdenabled", "0", "开启关闭转队提示", CONVAR_FLAGS_PLUGIN);
...
IpSql(ipnums)
{
	...
	if (hResult != INVALID_HANDLE) 
	{
		while (SQL_FetchRow(hResult)) 
		{
			...
			g_text = query;
            //CPrintToChatAll("{blue}[信息]:{olive}%s", query);
		}
		CloseHandle(hResult); 
	} 
}
...
{   
	...
	if (!IsFakeClient(client) && g_ClientPutInServer[client])
	{
		...
		{   
			CPrintToChatAll("{olive} %N {blue}加入游戏 {default}%s .\n {default}IP: {olive}%s  {default}来自:{olive}%s %s", client, SteamId, ClientIP, country, g_text);
			PrintToServer(" %N 加入游戏 %s  IP: %s  来自:%s %s %i", client, SteamId, ClientIP, country, g_text, ipnum);
		}
		else if(!IsFakeClient(client))
		{
			CPrintToChatAll("{olive} %N {blue}加入游戏 {default}%s .\n {default}IP: {olive}%s  {default}来自:{olive}",  client, SteamId, ClientIP);
			PrintToServer(" %N 加入游戏 %s  IP: %s  来自:局域网", client, SteamId, ClientIP);
		}
...
public Action:ipCommand(client, args)
{
	...
	ipnum = 0;
	CPrintToChat(client,"{olive} 输入 {red}!ip{default}在控制台查看自己信息");
	CreateTimer(0.5, Cooldown_Timer, client);
