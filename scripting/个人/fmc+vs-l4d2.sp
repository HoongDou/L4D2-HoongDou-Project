...
{
	CreateConVar("fmc_version", "2.4", "Force Mission Changer + Voting System Version", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCEnabled = CreateConVar("fmc_enable", "1", "Enable/Disable Plugin", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCIgnoreFail = CreateConVar("fmc_ignore_fail", "1", "Ignore/Mind Fail When Forcing Campaign Changes", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCAnnounce = CreateConVar("fmc_announce", "1", "Enable/Disable Announcements", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCVoteDuration = CreateConVar("fmc_vote_duration", "60", "Duration Of Campaign Voting", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCIncludeCurrent = CreateConVar("fmc_include_current", "0", "Include/Exclude Current Campaign From Being Voted", FCVAR_NOTIFY|FCVAR_SPONLY);
	hFMCIncludeLast = CreateConVar("fmc_include_last", "0", "Include/Exclude Last Campaign From Being Voted", FCVAR_NOTIFY|FCVAR_SPONLY);
	...
	AutoExecConfig(true, "fmc+vs-l4d2");
	...
	RegConsoleCmd("sm_mapvote", ShowVoteMenu, "Shows Menu Of Available And Vote-able Campaigns");
	RegConsoleCmd("sm_vcc", ShowVoteMenuCustom, "Shows Menu Of Available And Vote-able Custom Campaigns");
}
...
public Action ShowVoteMenu(int client, int args)
{
	...
	{
		PrintToChat(client, "\x04[SM]\x03 Finales Only!");
		return Plugin_Handled;
	}
	
	if (StrContains(sGameMode, "versus", false) != -1 && !GameRules_GetProp("m_bInSecondHalfOfRound"))
	{
		PrintToChat(client, "\x04[SM]\x03 Second Round Only!");
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "\x04[SM]\x03 Invalid Access!");
		return Plugin_Handled;
	}
	
	if (IsVoteInProgress())
	{
		PrintToChat(client, "\x04[SM]\x03 Vote In Progress!");
		return Plugin_Handled;
	}
...
public Action InformOfCC(Handle timer, any client)
{
	...
	PrintToChat(client, "\x04[SM]\x03 To Vote For Custom Campaigns, Type \x05!vcc");
	return Plugin_Stop;
}
...
{
	PrintToChatAll("\x04[SM]\x03 This Is The Finale!");
	if (!StrEqual(sVotedMap, "", false))
	{
		PrintToChatAll("\x04[SM]\x03 Next Map: %s |%s|", sVotedMap, sVotedCamp);
	}
	else
	{
		FMC_GetNextCampaign(sCurrentMap);
		PrintToChatAll("\x04[SM]\x03 Next Map: %s |%s|", sNextMap, sNextCamp);
	}
	return Plugin_Stop;
}
...
public Action VoteCampaignDelay(Handle timer)
{
	...
	PrintToChatAll("\x04[SM]\x03 Campaign Vote In 5..");
...
{
	PrintToChatAll("\x04[SM]\x03 Starting Campaign Vote!");
	FMCMenu();
...
)
{
	...
	PrintToChatAll("\x04[SM]\x03 Most Voted Campaign: %s (%s) \x05[%d Votes]", sVotedCamp, sVotedMap, item_info[majorityItem][VOTEINFO_ITEM_VOTES]);
...
void FMC_GetNextCampaign(const char[] sMap)
{
	...
	else if (StrEqual(sMap, "c14m2_lighthouse", false))
	{
		strcopy(sNextCamp, sizeof(sNextCamp), sStockCampNames[7]);
		strcopy(sNextMap, sizeof(sNextMap), sStockCamp[7]);
