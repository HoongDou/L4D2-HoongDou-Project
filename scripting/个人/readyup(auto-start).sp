...
new Handle:l4d_ready_autostart;//new-add;	
...
new footerCounter = 0;
new readyDelay;
new autoStartDelay;//new-add;
new bool:bIsGoingLiveFromAutoStart = false;//new-add;
...
public OnPluginStart()
{
	...
	l4d_ready_delay = CreateConVar("l4d_ready_delay", "3", "Number of seconds to count down before the round goes live.", 0, true, 0.0);//new-add;
...
public OnMapStart()
{
	...
	CancelLiveCountdown();
	AutoStartCountdownTimer = INVALID_HANDLE;//new-add;
...
public Action:ReadyCountdownDelay_Timer(Handle:timer)//new-add;
{
	if (readyDelay)
	{
		if (bIsGoingLiveFromAutoStart)
...
CancelFullReady()
{
	if (readyCountdownTimer != INVALID_HANDLE && !bIsGoingLiveFromAutoStart)//new-add;
	{
