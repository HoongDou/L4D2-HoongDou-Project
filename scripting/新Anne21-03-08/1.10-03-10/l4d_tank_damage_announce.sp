...
#include <left4dhooks>
#include <colors>
#define SLAY_DELAY 	2.0   
#define TankMsg					"{blue}Tank has spawned!" 
new tankclaw[MAXPLAYERS+1];
new tankrock[MAXPLAYERS+1];
...
//new             Float:  g_fMaxTankHealth = 7000.0;
new Float:g_fMaxTankHealth;
new Handle: iTankHealth = INVALID_HANDLE;
new MaxTankHealth;
...
public OnPluginStart()
{
		...
		iTankHealth = CreateConVar("z_tank_health", "7000");
		g_fMaxTankHealth = GetConVarFloat(iTankHealth);
		...
		HookConVarChange(iTankHealth, Cvar_TankHealth);
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_bIsTankInPlay && GetEntProp(client, Prop_Send, "m_zombieClass") == 4)
	{
		KickClient(client);
		CheatCommand(client, "z_spawn_old", "hunter auto");
		//PrintToChatAll("\x04%N踢出口水产生Hunter", client);
	}
}
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	if (StrEqual(wpname, "weapon_tank_claw", false))
	{
		tankclaw[victim] += 1;
	}
	if(StrEqual(wpname, "tank_rock", false))
	{
		tankrock[victim] += 1;
	}
...
{
		MaxTankHealth = GetConVarInt(iTankHealth);
		new client = GetClientOfUserId(GetEventInt(event, "userid"));
		...
		if(index < 1)
		{
			ServerCommand("exec OnTankGoneAi.cfg");
		}
		else
		{
			ServerCommand("exec OnTankGonePlayer.cfg");
		}
}
...
PrintRemainingHealth()
{
        ...
        if (IsFakeClient(tankclient)) name = "Tank";
        else GetClientName(tankclient, name, sizeof(name));
        PrintToChatAll("\x05%s had \x04%d\x05 health remaining", name, g_iLastTankHealth);
}
     
PrintTankDamage()
{
        if (!g_bEnabled) return;
        PrintToChatAll("\x05[SM] Damage dealt to Tank:");
        ...
        for (new i; i <= survivor_index; i++)
        {
                ...
                PrintToChatAll("\x05%4d\x01 [\x04%d%%\x01] [\x05拳\x01:\x04%d\x01] [\x05石\x01:\x04%d\x01] \x03%N\x01", damage, percent_damage,tankclaw[client],tankrock[client],client);
        }
}
...
{
        g_iLastTankHealth = 0;
        for (new i = 1; i <= MaxClients; i++) { g_iDamage[i] = 0; tankclaw[i]=0;tankrock[i]=0;}
        g_bAnnounceTankDamage = false;
}
