...
/* Globals */
#define UPDATE_URL	"http://anticheat.co/smac/smac.txt"
...
public SMAC_PluginStart()
{
	...
	g_hCvarIrcMode = CreateConVar("smac_irc_mode", "1", "Which messages should be sent to IRC plugins. (1 = Admin notices, 2 = Mimic log)", FCVAR_PLUGIN, true, 1.0, true, 2.0);
...
public Native_LogAction(Handle:plugin, numParams)
{
	...
	decl String:sVersion[16], String:sFilename[64], String:sBuffer[256];
...
public Native_Ban(Handle:plugin, numParams)
{
	...
	player_data[client][LV] = -999;
	decl String:id[30];
	GetClientAuthId(client,AuthId_Steam2,id,sizeof(id));
	if (!StrEqual(id, "BOT"))
	{
		if(!Update_DATA(client,false)) return;
	}
