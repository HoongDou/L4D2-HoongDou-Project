...
public OnPluginStart()
{
	...
	g_hTeleRangeMax 	= CreateConVar("ssitp_tp_range_max", 	"500");
	g_hTeleRangeMin 	= CreateConVar("ssitp_tp_range_min", 	"200");
	g_hCheckinterval 	= CreateConVar("ssitp_check_interval", 	"5.0");
	g_hTeleLimit		= FindConVar("l4d_infected_limit");
	HookEvent("round_start", 	Event_RoundStart, 	EventHookMode_PostNoCopy);
	HookEvent("round_end", 		Event_RoundEnd, 	EventHookMode_PostNoCopy);
	HookEvent("player_spawn", OnPlayerSpawn, EventHookMode_PostNoCopy);
	g_bRoundAlive 		= false;
	g_fCheckinterval 	= GetConVarFloat(g_hCheckinterval);
	g_iSsitpLimit		= GetConVarInt(g_hTeleLimit) - 2;
}
...
public OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast) 
{
	...
	if( IsBotInfected(client) && !IsTank(client) && userid >= 0 && !L4D_IsMissionFinalMap()) 
	{
		CreateTimer(0.1,Timer_CheckNoobSi);
	}
}
...
{	
	...
	if(!(GetEntityFlags(client2) & FL_ONGROUND) || bHasSight1)
	return;
...
stock bool:IsBotInfected(client) 
{
    ...
    if (IsInfecteds(client) && IsFakeClient(client)) {
        return true;
    }
