...
bool g_bCvarAllow, g_bMapStarted, g_bLateLoad, g_bLeft4Dead2, g_bHookedHurt;
...
int g_iLastOption[MAX_SPECIAL];
int g_iPinnedOption[MAX_SPECIAL];
int g_iIncapOption[MAX_SPECIAL];
int g_iVomsOption[MAX_SPECIAL];
float g_fWaitOption[MAX_SPECIAL];

float g_fLastSwitch[MAXPLAYERS+1];
int g_iLastAttacker[MAXPLAYERS+1];
int g_iLastOrders[MAXPLAYERS+1];
int g_iLastVictim[MAXPLAYERS+1];
bool g_bIncapped[MAXPLAYERS+1];
bool g_bLedgeGrab[MAXPLAYERS+1];
bool g_bPinBoomer[MAXPLAYERS+1];
bool g_bPinSmoker[MAXPLAYERS+1];
bool g_bPinHunter[MAXPLAYERS+1];
bool g_bPinJockey[MAXPLAYERS+1];
bool g_bPinCharger[MAXPLAYERS+1];
bool g_bPumCharger[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	g_hCvarType =			CreateConVar(	"l4d_target_override_type",				"1",				"How should the plugin search through Survivors. 1=Nearest visible (defaults to games method on fail). 2=All Survivors from the nearest.", CVAR_FLAGS );
...
void ExplodeToArray(char[] key, KeyValues hFile, int index, int arr[MAX_ORDERS])
{
	if( hFile.JumpToKey(key) )
	{
		...
		g_iPinnedOption[index] = hFile.GetNum("pinned");
		g_iIncapOption[index] = hFile.GetNum("incap");
		g_iVomsOption[index] = hFile.GetNum("voms");
		g_fWaitOption[index] = hFile.GetFloat("wait");
		g_iLastOption[index] = hFile.GetNum("last");
		hFile.Rewind();
	}
}
...
void HookPlayerHurt(bool doHook)
{
	...
	if( doHook && hook && !g_bHookedHurt )
	{
		HookEvent("player_hurt",		Event_PlayerHurt);
	}
	else if( (!doHook || !hook) && g_bHookedHurt )
	{
