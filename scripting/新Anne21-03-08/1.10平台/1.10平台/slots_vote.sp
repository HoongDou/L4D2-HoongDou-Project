...
public OnPluginStart()
{
        ...
        g_cvarSlotsPluginEnabled = CreateConVar("sm_slot_vote_enabled", "1", "Enabled?");
        g_cvarSlotsAutoconf = CreateConVar("sm_slot_autoconf", "1", "Autoconfigure slots vote max|min cvars?");
        g_hCVarMinAllowedSlots = CreateConVar("sm_slot_vote_min", "1", "Minimum allowed number of server slots (this value must be equal or lesser than sm_slot_vote_max).");
        g_hCVarMaxAllowedSlots = CreateConVar("sm_slot_vote_max", "16", "Maximum allowed number of server slots (this value must be equal or greater than sm_slot_vote_min).");
        g_cvarCurrentMaxSlots = CreateConVar("sv_maxslots", "4", "Maximum server slots.");
        ...
        if(GetConVarBool(g_cvarSlotsAutoconf)) {
                ...
                SetConVarInt(g_hCVarMinAllowedSlots, GetConVarInt(hSurvivorLimit) * 1);
                PrintToServer("Min slots automatically configured to %d", GetConVarInt(hSurvivorLimit) * 1);
                CloseHandle(hSurvivorLimit);
        }
        RegConsoleCmd("sm_slots", Cmd_SlotVote);
		RegConsoleCmd("sm_slot", Cmd_NoSpec);
}
...
{
        KickAllSpectators();
		ServerCommand("sm_cvar sv_maxslots 4");
        return Plugin_Stop;
}
...
static KickAllSpectators()
{
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
                if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 1)
                {
                        ...
                        BanClient(i, 10, BANFLAG_AUTHID, reason, reason, "nospec");
                        iSpecs++;
                }
        }
