...
// Tankhud Delayshow Var
ConVar cDelayShowTankHud_Time;
...
public void OnPluginStart()
{
	...
	cDelayShowTankHud_Time  = CreateConVar("l4d2_delayshow_tankhud_time", "10.0", "The time for delay showing tankhud for tank");
	...
	if (FillTankInfo(tankHud, true)) // No tank -- no HUD
	{
		for (int i = 1; i <= MaxClients; ++i)
		{
			...
			if (GetEntProp(i, Prop_Send, "m_zombieClass")!=8)
				...
				{
				CreateTimer(cDelayShowTankHud_Time.FloatValue, DelayShowTankHud, _, TIMER_DATA_HNDL_CLOSE);
				}
...
public Action DelayShowTankHud(Handle timer, any data)
{	
	Panel tankHud = new Panel();
	if (FillTankInfo(tankHud, true)) // No tank -- no HUD
	{
