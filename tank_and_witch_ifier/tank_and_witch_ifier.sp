...
public Action:AdjustBossFlow(Handle:timer) {
    ...
    if (!GetTrieValue(hStaticTankMaps, sCurMap, dummy)) {
        ...
        new iMinBanFlowW = L4D2_GetMapValueInt("witch_ban_flow_min", -1);
        new iMaxBanFlowW = L4D2_GetMapValueInt("witch_ban_flow_max", -1);
        new iMinBanFlowWB = L4D2_GetMapValueInt("witch_ban_flow_min_b", -1);
        new iMaxBanFlowWB = L4D2_GetMapValueInt("witch_ban_flow_max_b", -1);
        new iMinBanFlowWC = L4D2_GetMapValueInt("witch_ban_flow_min_c", -1);
        new iMaxBanFlowWC = L4D2_GetMapValueInt("witch_ban_flow_max_c", -1);
        ...
        PrintDebug("[AdjustBossFlow] flow: (%i, %i). ban (%i, %i). ban b (%i, %i). ban c (%i, %i)", iCvarMinFlow, iCvarMaxFlow, iMinBanFlowW, iMaxBanFlowW, iMinBanFlowWB, iMaxBanFlowWB, iMinBanFlowWC, iMaxBanFlowWC);
		
        // check each array index to see if it is within a ban range
        new iValidSpawnTotal = 0;
        for (new i = 0; i <= 100; i++) {
            bValidSpawn[i] = (iCvarMinFlow <= i && i <= iCvarMaxFlow) && !(iMinBanFlow <= i && i <= iMaxBanFlow) && !(iMinBanFlowB <= i && i <= iMaxBanFlowB) && !(iMinBanFlowC <= i && i <= iMaxBanFlowC) && !(iMinBanFlowW <= i && i <= iMaxBanFlowW) && !(iMinBanFlowWB <= i && i <= iMaxBanFlowWB) && !(iMinBanFlowWC <= i && i <= iMaxBanFlowWC);
        ...
        if (iValidSpawnTotal == 0) {
            ...
            PrintDebug("[AdjustBossFlow] Ban range covers entire flow range. Flow boss disabled.");
        }
        else {
            new n = Math_GetRandomInt(1, iValidSpawnTotal);
            new m = Math_GetRandomInt(1, iValidSpawnTotal);
        ...
        L4D2Direct_SetVSWitchToSpawnThisRound(0, false);
        L4D2Direct_SetVSWitchToSpawnThisRound(1, false);
        PrintDebug("[AdjustBossFlow] Static witch map. Flow witch disabled.");
    }
...
stock PrintDebugInfoDump() {
    if (GetConVarBool(g_hCvarDebug)) {
        ...
        PrintDebug("[Round 2] tank enabled: %i, tank flow: %f, display: %f, witch enabled: %i ,witch flow: %f, display: %f", L4D2Direct_GetVSTankToSpawnThisRound(1), L4D2Direct_GetVSTankFlowPercent(1), GetTankFlow(1), L4D2Direct_GetVSWitchToSpawnThisRound(1), L4D2Direct_GetVSWitchFlowPercent(1), GetWitchFlow(1));
    }
}
